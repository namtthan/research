const state = { view: "daily", query: "", topic: "All", data: { daily: [], weekly: [] } };
...
async function loadData() {
  try {
    const [daily, weekly] = await Promise.all([
      fetch("data/daily.json").then(r => { if (!r.ok) throw new Error("Could not load daily briefing data"); return r.json(); }),
      fetch("data/weekly.json").then(r => { if (!r.ok) throw new Error("Could not load weekly shortlist data"); return r.json(); })
    ]);
...
function currentItems() { return [...state.data[state.view]].sort((a,b) => b.date.localeCompare(a.date)); }

function storySearchText(story) {
  return [story.title, story.takeaway, story.what_happened, story.why_it_matters, story.caveat, ...(story.tags || []), story.method || "", story.read_first || ""].join(" ").toLowerCase();
}
...
function visibleGroups() {
  ...
  return currentItems().map(group => ({
    ...
    stories: (group.stories || []).filter(story => {
      const topicOK = state.topic === "All" || (story.tags || []).includes(state.topic);
...
function availableTags() {
  const tags = new Set();
  currentItems().forEach(g => (g.stories || []).forEach(s => (s.tags || []).forEach(t => tags.add(t))));
...
function renderFilters() {
  ...
  availableTags().forEach(tag => {
    ...
    btn.textContent = tag;
    btn.addEventListener("click", () => { state.topic = tag; render(); });
...
function storyCard(story, index) {
  ...
  const methodBlock = story.method ? `<h3>Method / model</h3><p>${escapeHTML(story.method)}</p>` : "";
  const readFirstBlock = story.read_first ? `<h3>Read first</h3><p>${escapeHTML(story.read_first)}</p>` : "";
  const sources = (story.sources || []).map(s => `<li><a href="${escapeAttribute(s.url)}" target="_blank" rel="noopener noreferrer">${escapeHTML(s.label || s.url)}</a></li>`).join("");
...
els.tabs.forEach(tab => tab.addEventListener("click", () => {
  ...
  state.topic = "All";
  els.tabs.forEach(t => { const active = t === tab; t.classList.toggle("active", active); t.setAttribute("aria-selected", String(active)); });
  render();
}));
